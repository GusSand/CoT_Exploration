#!/usr/bin/env python3
"""
Top-K Projection Intervention Comparison for CODI-LLaMA
Tests how projection onto top-k vocabulary embeddings affects CoT reasoning accuracy

Tests projection@k for k = [1, 2, 3, 5, 8, 10, 15, 20, 30, 50]
Also includes baseline and average ablation for comparison.

Intervention types:
1. Baseline (no intervention)
2. Average ablation (position-specific means from GSM8K train)
3. Projection@k (numbers only) - for each k value

Total conditions: 1 baseline + 1 average + 10 k values = 12 conditions
Tested on 3 datasets: Clean (132), GSM8K Train (132), GSM8K Test (132)
"""
...
def apply_intervention(A, token_id, embedding_layer, intervention_type,
    ...
    if intervention_type == 'zero':
    ...
    elif intervention_type.startswith('proj'):
        # Projection@k: extract k from intervention_type string (e.g., 'proj5' -> k=5)
        k_str = intervention_type[4:]  # Remove 'proj' prefix
        k = int(k_str)

        if k == 1:
            # Special case for k=1: use predicted token directly
            predicted_embd = embedding_layer(torch.tensor([token_id], device=A.device))
        ...
        # Always use normalized projection
...
def main():
    ...
    # Dataset 3: GSM8K test first 132
    ...
    intervention_conditions = [
        ('baseline', 'none'),  # Baseline (no intervention)
        ('average', 'numbers'),  # Average ablation baseline (for comparison)
    ]

    # Add projection@k for each k value (numbers only)
    for k in K_VALUES:
        intervention_conditions.append((f'proj{k}', 'numbers'))

    print(f"K values tested: {K_VALUES}")

    print(f"\n✓ Testing {len(intervention_conditions)} intervention conditions")

    # Test on all 132 examples from all three datasets
    num_test_clean = len(clean_dataset)  # 132
    num_test_train = len(gsm8k_train_132)  # 132
    num_test_test = len(gsm8k_test_132)  # 132

    print(f"\n" + "="*80)
    print(f"FULL EVALUATION: Testing All Examples")
    print(f"Clean Dataset: {num_test_clean} examples")
    print(f"GSM8K Train: {num_test_train} examples")
    print(f"GSM8K Test: {num_test_test} examples")
    print("="*80)

    # Run on all three datasets
    for dataset_name, dataset, num_examples in [
        ('clean', clean_dataset, num_test_clean),
        ('gsm8k_train', gsm8k_train_132, num_test_train),
        ('gsm8k_test', gsm8k_test_132, num_test_test)
    ]:
        print(f"\n{'='*80}")
