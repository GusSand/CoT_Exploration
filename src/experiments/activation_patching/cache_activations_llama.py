"""
Activation Caching Script for LLaMA
Extracts and caches hidden state activations from [THINK] tokens at multiple layers.

Adapted from cache_activations.py for LLaMA-3.2-1B-Instruct architecture.

Usage:
    python cache_activations_llama.py --model_path ~/codi_ckpt/llama_commonsense/gsm8k... --test
"""
...
LAYER_CONFIG = {
    'early': 4,    # Early layer (25% through 16-layer LLaMA)
...
class ActivationCacherLLaMA:
    ...
    def __init__(self, model_path: str, device: str = 'cuda'):
        """Initialize the cacher with CODI LLaMA model.
        ...
        model_args, data_args, training_args = parser.parse_args_into_dataclasses(
            args=[
                '--model_name_or_path', 'meta-llama/Llama-3.2-1B-Instruct',  # LLaMA specific
                '--output_dir', './tmp',
                '--num_latent', '6',
                '--use_lora', 'True',
                '--ckpt_dir', model_path,
                '--use_prj', 'True',
                '--prj_dim', '2048',  # LLaMA uses 2048, not GPT-2's 768
                '--lora_r', '128',
                '--lora_alpha', '32',
                '--lora_init', 'True',
            ]
        ...
        print(f"  Layers: 16 (caching from L{LAYER_CONFIG['early']}, L{LAYER_CONFIG['middle']}, L{LAYER_CONFIG['late']})")
    ...
    def cache_problem_activations(
        ...
        Args:
            ...
            layer_indices: Dict mapping layer names to indices (e.g., {'early': 4})
...
def main():
    ...
    parser.add_argument('--model_path', type=str, required=True, help='Path to CODI LLaMA checkpoint')
    ...
    if args.test:
        ...
        # Verify shape (LLaMA hidden dim = 2048)
        expected_shape = torch.Size([1, 2048])
        for layer_name, activation in activations.items():
            assert activation.shape == expected_shape, f"Expected shape {expected_shape}, got {activation.shape}"

        print(f"\n✓ All activations have correct shape: {expected_shape}")

        # Test save/load
        test_dir = "./results/activations_test_llama"
