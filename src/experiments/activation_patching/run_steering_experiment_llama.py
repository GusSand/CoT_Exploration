#!/usr/bin/env python3
"""
Run LLaMA Steering Experiment

Tests activation steering on LLaMA by adding alpha * direction to continuous thoughts.
Tests multiple alpha values and all three layers.

Usage:
    python run_steering_experiment_llama.py
"""
...
def main():
    ...
    print("="*80)
    print("LLAMA STEERING EXPERIMENT")
    print("="*80)
    ...
    for layer_name in ['early', 'middle', 'late']:
        ...
        direction_file = Path(__file__).parent / 'results' / 'steering_activations_llama' / layer_name / 'steering_direction.pt'
        direction_data = torch.load(direction_file)
        direction = direction_data['direction']

        print(f"\nSteering direction norm: {direction_data['direction_norm']:.4f}")
        print(f"Testing {len(alphas)} alpha values on {len(test_problems)} problems")
        ...
        output_dir = Path(__file__).parent / 'results' / 'steering_experiments_llama'
