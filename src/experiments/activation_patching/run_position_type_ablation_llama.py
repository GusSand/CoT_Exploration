#!/usr/bin/env python3
"""
Position-type ablation experiment for LLaMA.
...
def run_llama_position_ablation(max_samples=None):
    """Run LLaMA position-type ablation experiment."""
    print("=" * 70)
    print("LLAMA POSITION-TYPE ABLATION EXPERIMENT")
    print("=" * 70)
    ...
    output_file = Path("/home/paperspace/dev/CoT_Exploration/src/experiments/gpt2_token_ablation/results/llama_position_ablation.json")
    ...
    for sample in tqdm(samples, desc="LLaMA Position Ablation"):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--test', action='store_true', help='Run on 50 samples only')
    args = parser.parse_args()

    max_samples = 50 if args.test else None
    run_llama_position_ablation(max_samples=max_samples)
