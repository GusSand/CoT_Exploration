#!/usr/bin/env python3
"""
Position-type ablation experiment at LAST/LATE layer for LLaMA.

Ablate positions based on whether they decode to numbers vs non-numbers.
Tests causal importance of "number-encoding" positions at the last layer.

NOTE: LLaMA 'late' layer is L14, not L15 (true last). This is close to last.
"""
...
def run_llama_position_ablation_last_layer(max_samples=None):
    """Run LLaMA position-type ablation at LATE layer (L14, near-last)."""
    print("=" * 70)
    print("LLaMA POSITION-TYPE ABLATION - LATE LAYER (L14)")
    print("=" * 70)
    print("NOTE: L14 is 'late' layer (not true last L15, but close)")
    print("=" * 70)

    # Paths
    model_path = str(Path.home() / 'codi_ckpt' / 'llama_gsm8k')
    decoding_file = Path("/home/paperspace/dev/CoT_Exploration/src/experiments/gpt2_token_ablation/results/llama_final_layer_decoding.json")
    output_file = Path("/home/paperspace/dev/CoT_Exploration/src/experiments/gpt2_token_ablation/results/llama_position_ablation_last_layer.json")
    ...
    for sample in tqdm(samples, desc="LLaMA Late Layer Ablation"):
        ...
        # Condition A: Ablate number positions at LATE layer
        ablate_numbers_correct = None
        if len(number_positions) > 0:
            output_a = ablate_positions(cacher, patcher, question, number_positions, layer_name='late')
            pred_a = extract_answer_number(output_a)
            ablate_numbers_correct = answers_match(pred_a, ground_truth)
            stats['n_has_numbers'] += 1

        # Condition B: Ablate non-number positions at LATE layer
        ablate_non_numbers_correct = None
        if len(non_number_positions) > 0:
            output_b = ablate_positions(cacher, patcher, question, non_number_positions, layer_name='late')
    ...
    output = {
        'model': 'LLaMA-3.2-1B',
        'ablation_layer': 'late (L14 - near last, not true last L15)',
    ...
    # Print results
    print("\n" + "=" * 70)
    print("LLaMA LATE LAYER RESULTS")
    print("=" * 70)
...
if __name__ == "__main__":
    ...
    run_llama_position_ablation_last_layer(max_samples=max_samples)
