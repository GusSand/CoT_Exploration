...
def run_cot_with_causal_intervention(model, tokenizer, training_args, question, target_token='5', k=3):
    """Run CoT WITH CAUSAL intervention"""
    batch_size = 1
    questions = [question]
...
def process_example(model, tokenizer, training_args, question, answer, target_token='5', k=3):
    """Process a single example with and without intervention"""
    result = {
        'question': question,
        'ground_truth': answer,
...
def main():
    ...
    with open('/workspace/CoT_Exploration/src/experiments/28-10-2028-projection-replacement/projection_replacement_clean/llama_cot_clean.json', 'r') as f:
        dataset = json.load(f)

    print(f"✓ Loaded {len(dataset)} clean variant examples")
    ...
    for idx, example in enumerate(tqdm(dataset)):
        ...
        try:
            ...
            # Add metadata from original dataset
            result['pair_id'] = example['pair_id']
            result['variant'] = example['variant']
            result['reasoning_steps'] = example['reasoning_steps']
            result['baseline_correct'] = example['baseline_correct']
    ...
    print(f"\nAnswers that changed due to intervention: {answers_changed}/{total} ({answers_changed/total*100:.1f}%)")

    output_dir = Path("/workspace/CoT_Exploration/src/experiments/28-10-2028-projection-replacement/projection_replacement_clean/results")
    output_dir.mkdir(exist_ok=True)

    results_file = output_dir / "llama_intervention_results_clean.json"
