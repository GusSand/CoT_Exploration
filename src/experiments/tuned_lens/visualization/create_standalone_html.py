...
This eliminates the CORS/fetch issue when opening as a local file.
"""

import json
import sys

def create_standalone_html(json_path, output_path):
...
<head>
    ...
    <title>Layer Probe Visualization - 2-Chunk Training</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        <h1>🔬 Layer Probe Visualization</h1>
        <div class="subtitle">2-Chunk Training • Tuned Lens Analysis</div>
        ...
        <div id="loadingMessage" class="loading">Loading visualization...</div>
    </div>
    ...
    <script>
        ...
        // Layer performance metadata (from evaluation)
        const layerMetadata = {
            2: { badge: 'best', acc: 74 },
            6: { badge: 'best', acc: 73 },
            7: { badge: 'best', acc: 72 },
        ...
        function render() {
            ...
            // Get L11's prediction for matching
        ...
        function createLayerCard(layer, l11Token, layerIdx) {
            ...
            // Apply special styling based on metadata
            if (layerIdx === 11) {
            ...
            // Tuned lens predictions (if available)
        ...
        function createPredictionSection(title, predictions, l11Token) {
            ...
            // Show only top 3 for compactness
    ...
    print(f"Writing standalone HTML to: {output_path}")
    with open(output_path, 'w', encoding='utf-8') as f:
        f.write(html_content)

    print("✓ Standalone HTML created successfully!")
    print(f"\nYou can now open '{output_path}' directly in any browser.")
    print("No web server needed - the JSON data is embedded in the HTML file.")

if __name__ == '__main__':
    json_file = 'layer_probe_visualization.json'
    output_file = 'layer_probe_visualization_standalone.html'

    create_standalone_html(json_file, output_file)
