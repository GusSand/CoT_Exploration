...
def analyze_all_features():
    ...
    features_data = torch.load(
        'src/experiments/llama_feature_interpretability/data/llama_extracted_features.pt',
        weights_only=False
    )
    ...
    with open('src/experiments/llama_feature_interpretability/data/llama_cot_tokens.json', 'r') as f:
    ...
    # Convert problem IDs to array for efficient lookup
    ...
    # Summary
    TOTAL_FEATURES = 49152  # 96 SAEs × 512 features
    ...
    print(f"  Interpretable features: {num_interpretable_features:,} ({num_interpretable_features/TOTAL_FEATURES*100:.1f}%)")
    ...
    # Save results
    output_path = Path('src/experiments/llama_feature_interpretability/data/llama_feature_token_correlations.json')
    ...
    output_data = {
        'metadata': {
            ...
            'interpretability_rate': num_interpretable_features / TOTAL_FEATURES,
    ...
    print("="*80)
    print(f"  Interpretable features: {num_interpretable_features:,} / 49,152 ({num_interpretable_features/TOTAL_FEATURES*100:.1f}%)")
