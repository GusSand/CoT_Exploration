#!/usr/bin/env python3
"""
Phase 6: Sequential Ablation During Continuous Thought Iterations

Apply feature ablation during each of the 6 latent continuous thought iterations,
then compare final answer logits between baseline and ablated conditions.
"""
...
print("="*80)
print("PHASE 6: SEQUENTIAL ABLATION DURING CONTINUOUS THOUGHT")
print("="*80)
...
# Ablation hook - modified for 3D tensors only
class AblationHook:
    ...
    def __call__(self, module, input, output):
        ...
        # Only process if 3D tensor (should be [1, 1, 2048] during latent iterations)
...
def run_codi_generation(model, tokenizer, question, training_args,
                       ablate_during_latent=False, mult_features=None, sae=None):
    """
    Run full CODI generation.

    Args:
        ablate_during_latent: If True, apply ablation hooks only during latent iterations
        mult_features: Dict of {layer: [feature_ids]} to ablate
        sae: SAE model for ablation
    """
    ...
    with torch.no_grad():
        ...
        # Latent iterations - APPLY ABLATION HERE
        for i in range(training_args.inf_latent_iterations):
            # Register hooks for THIS iteration only if ablation enabled
            handles = []
            if ablate_during_latent and mult_features is not None and sae is not None:
                for layer in ['early', 'middle', 'late']:
                    layer_idx = {'early': 4, 'middle': 8, 'late': 14}[layer]
                    features_to_ablate = mult_features[layer]
        ...
        # Generate tokens (NO ABLATION HERE)
        pred_tokens = []
        all_logits = []
...
baseline_result = run_codi_generation(
    model, tokenizer, question, training_args,
    ablate_during_latent=False
)
...
# Run with ablation during latent iterations
print("\n[6/6] Running with multiplication feature ablation during latent iterations...")
ablated_result = run_codi_generation(
    model, tokenizer, question, training_args,
    ablate_during_latent=True,
    mult_features=mult_features_to_ablate,
    sae=sae
)
...
# Save results
output = {
    'experiment': 'sequential_ablation_during_continuous_thought',
    'description': 'Ablation applied during 6 latent continuous thought iterations only, not during visible token generation',
...
output_path = Path('./phase6_sequential_ablation_results.json')
