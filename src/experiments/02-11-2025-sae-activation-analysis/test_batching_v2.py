...
VERSION 2: Batch the encoding + CoT phase, but decode sequentially per item.
This avoids the past_key_values slicing issue.

Compares accuracy between:
1. Sequential processing (batch_size=1) - baseline
2. Batched encoding+CoT (batch_size=8, 16) with sequential decoding
"""
...
def run_batched_encoding_then_decode(model, tokenizer, training_args, questions, batch_size=8):
    """
    Run batched encoding + CoT, then decode each item sequentially.
    This avoids the past_key_values slicing issue.
    """
    ...
    # Now decode each item SEQUENTIALLY to avoid past_key_values issues
    results = []
    for batch_idx in range(actual_batch_size):
        # Extract this item's past_key_values
        batch_past_kv = tuple(
        ...
        with torch.no_grad():
            ...
            pred_tokens = []
            item_past_kv = batch_past_kv
            for step in range(128):
                out = model.codi(
                    inputs_embeds=output,
                    output_hidden_states=False,
                    use_cache=True,
                    past_key_values=item_past_kv
                )
...
def main():
    ...
    # Test 2: Batched encoding + CoT with batch_size=8
    print("\n" + "="*80)
    print("Test 2: Batched Encoding+CoT (batch_size=8), Sequential Decoding")
    print("="*80)

    batch_size_8_results = []
    for i in tqdm(range(0, len(questions), 8), desc="Batch-8"):
        batch_questions = questions[i:i+8]
        batch_results = run_batched_encoding_then_decode(model, tokenizer, training_args, batch_questions, batch_size=8)
    ...
    # Test 3: Batched encoding + CoT with batch_size=16
    print("\n" + "="*80)
    print("Test 3: Batched Encoding+CoT (batch_size=16), Sequential Decoding")
    print("="*80)

    batch_size_16_results = []
    for i in tqdm(range(0, len(questions), 16), desc="Batch-16"):
        batch_questions = questions[i:i+16]
        batch_results = run_batched_encoding_then_decode(model, tokenizer, training_args, batch_questions, batch_size=16)
    ...
    output_path = Path('./batching_test_results_v2.json')
    ...
    if batch8_accuracy >= sequential_accuracy - 1:
        print("✅ Batching encoding+CoT with batch_size=8 is VIABLE (< 1% accuracy drop)")
