...
class SparseAutoencoder(torch.nn.Module):
    """SAE model matching the training code"""
...
def load_sae():
    ...
    sae_path = "/workspace/CoT_Exploration/src/experiments/sae_pilot/refined/sae_weights.pt"
...
def run_cot_with_hidden_states(model, tokenizer, training_args, question):
    ...
    with torch.no_grad():
        ...
        # Store hidden state before projection
...
def analyze_sae_activations(sae, hidden_states_list, top_k=20):
    ...
    with torch.no_grad():
        for hs_info in hidden_states_list:
            ...
            # Use the projected hidden state (2048-dim) for SAE
...
def main():
    ...
    print(f"Captured {len(hidden_states_list)} hidden states")
    ...
    output_path = Path('./sae_activation_analysis.json')
