...
def load_activations():
    ...
    data_file = script_dir.parent / "data" / "processed" / "probe_activations_llama3b_proper.json"
...
def main():
    ...
    for layer in layers:
        ...
        for token_idx in range(n_tokens):
            # Extract features
            X_train = np.array([s['thoughts'][layer_key][token_idx] for s in train_samples])
            y_train = np.array([1 if s['is_honest'] else 0 for s in train_samples])

            X_test = np.array([s['thoughts'][layer_key][token_idx] for s in test_samples])
            y_test = np.array([1 if s['is_honest'] else 0 for s in test_samples])

            # Train probe
            result = train_probe(X_train, y_train, X_test, y_test, layer_key, token_idx)
