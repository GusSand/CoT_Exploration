...
def main():
    ...
    num_test = 5
    print(f"\n" + "="*80)
    print(f"Testing on First {num_test} Examples (Clean Dataset)")
    print("="*80)
    ...
    for condition_idx, (interv_type, interv_scope) in enumerate(intervention_conditions):
        ...
        for ex_idx in range(num_test):
            example = clean_dataset[ex_idx]
            question = example['question']
    ...
    # Save test results
    test_results_file = RESULTS_DIR / 'test_results_5_examples.json'
