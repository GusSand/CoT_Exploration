#!/usr/bin/env python3
"""
Few-shot evaluation of LLaMA-3.2-8B on Personal Relations Task.

Phase 1: Test 8B few-shot baseline to assess scaling potential.
"""
...
def main():
    print("=" * 80)
    print("PERSONAL RELATIONS - FEW-SHOT EVALUATION (3B MODEL)")
    print("=" * 80)
    ...
    # Save results
    output_file = 'few_shot_results_3b.json'
    with open(output_file, 'w') as f:
        json.dump(all_results, f, indent=2)

    print(f"\n{'=' * 80}")
    print("SUMMARY - 3B vs 1B COMPARISON")
    print("=" * 80)
    print(f"\n{'Setting':<25} {'3B Accuracy':<15} {'1B Accuracy':<15} {'Improvement':<10}")
    print("-" * 65)

    # Load 1B results for comparison
    try:
        with open('few_shot_results_v2.json', 'r') as f:
            results_1b = json.load(f)
        accs_1b = {(r['n_shot'], r['include_cot']): r['accuracy'] for r in results_1b}
    except:
        accs_1b = {}
    ...
    for result in all_results:
        ...
        acc_1b = accs_1b.get(key, 0)
        improvement = result['accuracy'] - acc_1b

        print(f"{setting:<25} {result['accuracy']:.1f}%{'':<11} {acc_1b:.1f}%{'':<11} +{improvement:.1f}%")

    print(f"\n{'=' * 80}")
    print(f"Results saved to: {output_file}")
    print(f"Paper baseline (LLaMA-3.3-70B): 88.4%")
    print(f"\nPHASE 1 DECISION POINT:")
    best_acc = max(r['accuracy'] for r in all_results)
    if best_acc >= 65.0:
        print(f"✅ SUCCESS: {best_acc:.1f}% >= 65% - PROCEED TO PHASE 2 (data generation)")
    else:
        print(f"⚠️  MARGINAL: {best_acc:.1f}% < 65% - Consider if Phase 2/3 worth investment")
    print(f"{'=' * 80}")
