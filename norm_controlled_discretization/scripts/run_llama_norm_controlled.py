#!/usr/bin/env python3
"""
CODI Discretization Analysis with NORM CONTROL - LLAMA VERSION
Compares: vanilla vs alternating discretization vs full discretization
KEY CHANGE: When discretizing, scale token embedding to match L2 norm of continuous vector
...
# Add src to path - go up to CoT_Exploration root
...
parser.add_argument("--output_dir", type=str, default="./llama_norm_controlled_results", help="Output directory")
...
def run_inference_batched(model, tokenizer, questions, training_args, discretize_mode="vanilla", device="cuda"):
    ...
    with torch.no_grad():
        ...
        for i in range(training_args.inf_latent_iterations):
            ...
            # Discretization logic with NORM CONTROL
            ...
            if should_discretize:
                # CRITICAL CHANGE: Compute L2 norm of continuous vector BEFORE discretization
                continuous_norms = torch.norm(latent_embd[:, 0, :], dim=-1)  # [batch]

                # Discretize entire batch
                top_tokens = torch.argmax(probs[:, 0, :], dim=-1)  # [batch]

                # Get token embeddings
                token_embd = model.get_embd(model.codi, model.model_name)(top_tokens.unsqueeze(1)).to(device)  # [batch, 1, hidden]

                # CRITICAL CHANGE: Scale token embeddings to match original norms
                token_norms = torch.norm(token_embd[:, 0, :], dim=-1)  # [batch]
                scale_factors = continuous_norms / (token_norms + 1e-8)  # [batch], add epsilon to avoid division by zero

                # Apply scaling
                latent_embd = token_embd * scale_factors.unsqueeze(1).unsqueeze(2)  # [batch, 1, hidden]
...
def main():
    print("="*80)
    print("CODI-LLAMA DISCRETIZATION ANALYSIS - NORM CONTROLLED VERSION")
    print("="*80)
    ...
    # Print summary
    print("\n" + "="*80)
    print("FINAL RESULTS - CODI-LLAMA NORM CONTROLLED")
    print("="*80)
    for mode in modes:
        s = stats[mode]
