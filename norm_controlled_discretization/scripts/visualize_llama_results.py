#!/usr/bin/env python3
"""
Visualize LLAMA Norm-Controlled Discretization Results
"""
import json
...
# Load results
with open("/workspace/CoT_Exploration/llama_norm_controlled_results/final_results.json", "r") as f:
    data = json.load(f)
...
import os
os.makedirs("/workspace/CoT_Exploration/norm_controlled_discretization/plots/llama", exist_ok=True)
...
ax.set_title("CODI-Llama Norm-Controlled Discretization: Accuracy Comparison", fontsize=14, fontweight="bold")
ax.set_ylim(0, 60)
...
plt.tight_layout()
plt.savefig("/workspace/CoT_Exploration/norm_controlled_discretization/plots/llama/accuracy_comparison.png", dpi=300, bbox_inches="tight")
...
ax.set_title("CODI-Llama Norm-Controlled: Confidence by Position", fontsize=14, fontweight="bold")
ax.set_xticks(x)
ax.set_xticklabels(position_labels)
ax.legend()
ax.grid(axis="y", alpha=0.3)
plt.tight_layout()
plt.savefig("/workspace/CoT_Exploration/norm_controlled_discretization/plots/llama/confidence_by_position.png", dpi=300, bbox_inches="tight")
...
ax.set_title("CODI-Llama Norm-Controlled: Token Diversity by Position", fontsize=14, fontweight="bold")
ax.set_xticks(x)
ax.set_xticklabels(position_labels)
ax.legend()
ax.grid(axis="y", alpha=0.3)
plt.tight_layout()
plt.savefig("/workspace/CoT_Exploration/norm_controlled_discretization/plots/llama/token_diversity.png", dpi=300, bbox_inches="tight")
...
fig.suptitle("CODI-Llama Norm-Controlled: Scale Factor Distributions", fontsize=16, fontweight="bold")
...
plt.tight_layout()
plt.savefig("/workspace/CoT_Exploration/norm_controlled_discretization/plots/llama/scale_factor_distribution.png", dpi=300, bbox_inches="tight")
print("Saved: scale_factor_distribution.png")
plt.close()

print("\nAll LLAMA visualizations completed!")
print(f"Saved to: /workspace/CoT_Exploration/norm_controlled_discretization/plots/llama/")
